    ...
    def _learn_XGBoost(self, principalComponents):
        ...
        for train_p in [70]:
            ...
            for num_splits in range(1, 101):
                ...
                params = {'silent': True, 'gamma': 0.4, 'max_depth': 7, 'lambda': 2.5, 'eta': 0.18}
                clf_xgb = xgb.train(params, dtrain=dtrain, evals=[(dtrain, 'train'), (deval, 'eval')],
                                    early_stopping_rounds=10, verbose_eval=False)
                y_score_test = clf_xgb.predict(dtest, ntree_limit=100)
                y_score_train = clf_xgb.predict(dtrain, ntree_limit=100)
